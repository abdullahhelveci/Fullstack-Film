...
const keys = ['title','year','rating','description','language','director','duration'];
  ...
  if ((req.url = "/api/movies")) {
    ...
    if (
      ...
      !body.cast.length > 0
        ...
        res.writeHead(404)
        res.end('lütfen zorunlu olan bütün alanları tamamlayın')
        return
    }
      // kaydedilecek filme id ekle
      body.id = crypto.randomUUID()

      // json dosyasaından verileri al
      let data = fs.readFileSync('./data/movies.json','utf-8')
      data = JSON.parse(data)

      // mevcut filmerin üzerine yeni film ekle
      data.push(body)
